--[[
# See https://wiki.hyprland.org/Configuring/Window-Rules/ for more
windowrule = suppress_event maximize, match:class .*
# windowrule = match:class .*, suppressevent maximize

# Just dash of opacity by default
windowrule = opacity 0.97 0.9, match:class .*
# windowrule = opacity 0.97 0.9, class:.*

# Fix some dragging issues with XWayland
# windowrule = nofocus, match:class ^$,match:title ^$,xwayland:1,floating:1,fullscreen:0,pinned:0
# windowrule = no_focus, match:class ^$, match:title ^$, match:xwayland 1, match:floating 1, match:fullscreen 0, match:pinned 0
# windowrule = match:class ^$, match:title ^$, match:xwayland 1, match:float 1, match:fullscreen 0, match:pin 0, no_focus
windowrule = no_focus on, match:class ^$, match:title ^$, match:xwayland 1, match:float 1, match:fullscreen 0, match:pin 0


windowrule = workspace 3, match:class obsidian
windowrule = group set always, match:class obsidian

# https://wiki.hypr.land/Configuring/Window-Rules/#tags
windowrule = tag +msg, match:class .*(discord|signal|telegram).*
windowrule = workspace 4, match:tag msg
windowrule = group set always, match:tag msg
# windowrule = group new lock always, match:tag msg

windowrule = workspace 5, match:class spotify


# windowrule = monitor DVI-I-4, match:class steam_app_0
# windowrule = size 1920 1080, match:class steam_app_0

# # Example rules for a window with title "overlay"
# # windowrulev2 = pin,title:overlay
# windowrulev2 = float,title:foot-overlay
# windowrulev2 = center,title:foot-overlay
# windowrulev2 = move 20% 70%,title:foot-overlay
# windowrulev2 = noborder,title:foot-overlay
# windowrulev2 = nofocus,title:foot-overlay
# windowrulev2 = size 60% 20%,title:foot-overlay
#
# # windowrulev2 = float,class:.*showmethekey-gtk.*
# windowrulev2 = float,title:Floating Window - Show Me The Key
# windowrulev2 = center,title:Floating Window - Show Me The Key
# windowrulev2 = move 70% 90%,title:Floating Window - Show Me The Key
# windowrulev2 = noborder,title:Floating Window - Show Me The Key
# windowrulev2 = nofocus,title:Floating Window - Show Me The Key
# windowrulev2 = size 20% 5%,title:Floating Window - Show Me The Key

# # Example rules for a window with title "overlay"
# # windowrulev2 = pin,title overlay
# windowrule = float on, match:title foot-overlay
# windowrule = center on, match:title foot-overlay
# windowrule = move 70% 90%, match:title foot-overlay
# # windowrule = noborder on, match:title foot-overlay
# windowrule = no_focus on, match:title foot-overlay
# windowrule = size monitor_h*0.1 5%, match:title foot-overlay

# windowrulev2 = float,class:.*showmethekey-gtk.*
windowrule = float on, match:title Floating Window - Show Me The Key
windowrule = center on, match:title Floating Window - Show Me The Key
windowrule = move (monitor_w*0.4) (monitor_h*0.9), match:title Floating Window - Show Me The Key
# windowrule = noborder,match:title Floating Window - Show Me The Key
windowrule = no_focus on, match:title Floating Window - Show Me The Key
windowrule = size (monitor_w*0.2) (monitor_h*0.05), match:title Floating Window - Show Me The Key
windowrule = pin on, match:title Floating Window - Show Me The Key
]]
--

-- See https://wiki.hyprland.org/Configuring/Window-Rules/ for more
hl.window_rule({
	name = "suppress-maximize-events",
	match = { class = ".*" },
	suppress_event = "maximize",
})

-- Just dash of opacity by default
hl.window_rule({
	name = "suppress-maximize-events",
	match = { class = ".*" },
	opacity = "0.97 0.9",
})

-- Fix some dragging issues with XWayland
hl.window_rule({
	name = "fix-xwayland-drags",
	match = {
		class = "^$",
		title = "^$",
		xwayland = true,
		float = true,
		fullscreen = false,
		pin = false,
	},

	no_focus = true,
})

-- windowrule = workspace 3, match:class obsidian
hl.window_rule({
	match = {
		class = "obsidian",
	},
	workspace = "3",
})
-- windowrule = group set always, match:class obsidian
hl.window_rule({
	match = {
		class = "obsidian",
	},
	group = "set always",
})

-- https://wiki.hypr.land/Configuring/Window-Rules/#tags
-- windowrule = tag +msg, match:class .*(discord|signal|telegram).*
hl.window_rule({
	match = {
		class = ".*(discord|signal|telegram).*",
	},
	tag = "+msg",
})
-- windowrule = workspace 4, match:tag msg
-- windowrule = group set always, match:tag msg
hl.window_rule({
	match = {
		tag = "msg",
	},
	workspace = "4",
	group = "set always",
})

-- windowrule = workspace 5, match:class spotify
hl.window_rule({
	match = {
		class = "spotify",
	},
	workspace = "5",
})

-- windowrule = float on, match:title Floating Window - Show Me The Key
-- windowrule = center on, match:title Floating Window - Show Me The Key
-- windowrule = move (monitor_w*0.4) (monitor_h*0.9), match:title Floating Window - Show Me The Key
-- windowrule = no_focus on, match:title Floating Window - Show Me The Key
-- windowrule = size (monitor_w*0.2) (monitor_h*0.05), match:title Floating Window - Show Me The Key
-- windowrule = pin on, match:title Floating Window - Show Me The Key
hl.window_rule({
	match = {
		title = "Floating Window - Show Me The Key",
	},
	float = true,
	center = true,
	move = { "monitor_w*0.4", "monitor_h*0.9" },
	no_focus = true,
	size = { "monitor_w*0.2", "monitor_h*0.05" },
	pin = true,
})
